...
Two modes:
  ...
  1. Change ref (--ref): re-deploy the app-of-apps at a new git ref (branch or
     release tag), then let ArgoCD roll it out. This re-runs the idempotent
     installer against the existing cluster.
...
// runUpgradeChangeRef re-deploys the platform at a new ref by re-running the
// idempotent installer (Mode 1). Upgrades always force so an unchanged release
// is still reconciled.
...
func runUpgradeForceSync(cmd *cobra.Command, args []string, flags *InstallFlags, verbose bool) error {
	...
	if flags.DryRun {
		...
		return sharedErrors.HandleGlobalError(previewOutOfSync(cmd.Context(), manager, verbose), verbose)
	}

	pterm.Info.Println("Refreshing and syncing the OpenFrame platform...")
	if err := manager.RefreshAndSync(cmd.Context()); err != nil {
		return sharedErrors.HandleGlobalError(err, verbose)
	}
...
// triggering a sync (Mode 2 --dry-run).
func previewOutOfSync(ctx context.Context, manager *argocd.Manager, verbose bool) error {
